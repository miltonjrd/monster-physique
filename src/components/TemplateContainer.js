...
const Image = ({ side, imgProps }) => {
  ...
  return (
    <ResizableBox 
      ...
      style={{
        left: imgProps.position.x,
        top: imgProps.position.y
      }}
    ...
    >
      ...
      <img src={URL.createObjectURL(imgProps.file)} alt="Imagem" draggable="false" />
    </ResizableBox>
  )
};
  ...
  return (
    ...
    <Container>
      <div className="d-flex gap-3 justify-content-center">
        <div className={!currentSide && 'hide-on-tablet'}>
          <div className="position-relative text-center" style={{ flex: 1 }}>
            ...
            <img className='template-img' src={ShirtFront} alt="" tabIndex={-1} />
            <div
              className="position-absolute top-0 w-100 h-100"
              style={{
                WebkitMaskImage: `url(${ShirtFront})`,
                WebkitMaskSize: '100%'
              }}
            >
              {
                simulatorContext.images.front.map((image) => (
                  <Image key={image.id} side="front" imgProps={image} />
                ))
              }
            </div>
          </div>
        ...
        <div className={currentSide && 'hide-on-tablet'}>
          <div className="position-relative text-center" style={{ flex: 1 }}>
            ...
            <img className='template-img' src={ShirtBack} alt="" tabIndex={-1} />
            <div
              className="position-absolute top-0 w-100 h-100"
              style={{
            ...
            >
              {
                simulatorContext.images.back.map((image) => (
                  <Image key={image.id} side="back" imgProps={image} />
                ))
              }
            </div>
          </div>
      ...
      <div className="d-flex align-self-center gap-3">
        <button 
          className="btn btn-danger text-white px-4 align-self-center"
          onClick={() => setResetModalShow(true)}
        >
          <strong>Resetar mudanças</strong>
        </button>
        <button
          className="show-on-tablet btn btn-dark border-0 rounded text-white p-2 align-self-end rounded-circle"
